  ...
  <div class="shopcart">
    ...
    <div class="bottomNav-cartfooter" :class="{'bottomNav-carticon-empty':isEmpty}">
      ...
      <div class="bottomNav-cartInfo" @click="showCartView = !showCartView">
        ...
        <p v-if="!isEmpty" class="bottomNav-cartdelivery">另需配送费{{numPirce}}元</p>
      </div>
      <button class="submit-btn">
        <span v-if="isEmpty">¥{{numPirce}}元起送</span>
...
export default {
  ...
  created() {
    ...
    this.numPirce = data.float_delivery_fee;
  },
  methods: {
    // 清空
    clearFoods() {
    ...
    settlement() {
      ...
      Promise.resolve(this.$router.push("/settlement")).then(() => {
        Indicator.close();
      });
    }
  }
