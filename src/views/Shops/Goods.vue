<template>
  ...
  <div class="goods" v-if="shopInfo">
    ...
    <!-- 商品详情 -->
    <food-info :food="selectedFood" :isShow="showFood" @close="showFood = false"></food-info>
  </div>
</template>
...
import FoodInfo from "@/components/Shops/Foodinfo.vue";
...
export default {
  ...
  methods: {
    // 商家列表 滚动商品列表 数据获取
    getData() {
      // 只有单个数据 没有在从shopList.vue 接受其他内容
      this.$axios.get(`/api/profile/shopInfo`).then(res => {
